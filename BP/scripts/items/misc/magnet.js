...
world.afterEvents.itemUse.subscribe(event => {
    ...
    entity.runCommand('replaceitem entity @p slot.weapon.mainhand 0 complementary:magnet_active 1 0')
    entity.runCommand('playsound random.click @a ~~~ 1 0.8')
})

const magnet_active = [
    'complementary:magnet_active'
]

world.afterEvents.itemUse.subscribe(event => {
    const { source: entity, itemStack } = event
    if (entity.typeId !== "minecraft:player" || !itemStack || !magnet_active.includes(itemStack.typeId)) return;
    entity.runCommand('replaceitem entity @p slot.weapon.mainhand 0 complementary:magnet 1 0')
